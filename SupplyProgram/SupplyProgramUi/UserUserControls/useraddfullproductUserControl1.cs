...
{
    public partial class useraddfullproductUserControl1 : UserControl
    {
        Normaluser normaluser = new Normaluser();
        public event Action<string> ProductChanged;
        public event Action SaveAndExit;
        public Action<ComboBox, ComboBox, ComboBox> fillcomboboxes = (locationbox, productbox, packagebox) =>
        {
        ...
        public Action<ComboBox, TextBox> updatescalebox = (packagebox, scaletext) =>
        {
            using (var db = new SuplyProgramContext())
            {
                if (packagebox != null)
